  ...
  return (
    <section className="bg-black grid grid-cols-1 md:grid-cols-2 min-h-96 text-white px-6 md:px-12 py-8 md:py-14 my-10">
      ...
      <div className="flex gap-y-5 flex-col justify-between md:justify-start items-start  ">
        ...
        {/* timer */}
        <div className="text-black flex gap-3 md:gap-5 items-center">
          <div className="rounded-full bg-white w-13 h-13 md:w-20 md:h-20 flex gap-2 flex-col justify-center items-center  ">
            <p className="font-bold text-sm md:text-xl">
              {String(days).padStart(2, '0')}
            </p>
            <p className="text-xs md:text-sm capitalize">{t('days')}</p>
          </div>
          <div className="rounded-full bg-white w-13 h-13 md:w-20 md:h-20 flex gap-2 flex-col justify-center items-center  ">
            <p className="font-bold text-sm md:text-xl">
              {String(hours).padStart(2, '0')}
            </p>
            <p className="text-xs md:text-sm capitalize">{t('hours')}</p>
          </div>
          <div className="rounded-full bg-white w-13 h-13 md:w-20 md:h-20 flex gap-2 flex-col justify-center items-center  ">
            <p className="font-bold text-sm md:text-xl">
              {String(minutes).padStart(2, '0')}
            </p>
            <p className="text-xs md:text-sm capitalize">{t('minutes')}</p>
          </div>
          <div className="rounded-full bg-white w-13 h-13 md:w-20 md:h-20 flex gap-2 flex-col justify-center items-center  ">
            <p className="font-bold text-sm md:text-xl">
              {String(seconds).padStart(2, '0')}
            </p>
            <p className="text-xs md:text-sm capitalize">{t('seconds')}</p>
          </div>
        </div>
